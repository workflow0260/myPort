...
export const CinematicNavbar: React.FC<NavbarProps> = ({ onOpenResumeModal }) => {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 flex justify-center px-4 pt-3 md:pt-4 transition-all duration-300 pointer-events-none">
      <div
        className={`pointer-events-auto flex items-center justify-between gap-4 rounded-full border border-[rgba(17,17,16,0.08)] bg-[rgba(251,251,251,0.92)] backdrop-blur-2xl shadow-[0_1px_0_rgba(255,255,255,0.8)_inset,0_8px_24px_-18px_rgba(23,55,83,0.25)] transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] ${isScrolled
      ...
      >
        ...
        >
          <span className="relative grid h-5 w-5 place-items-center shrink-0">
            <svg viewBox="0 0 24 24" fill="none" className="h-5 w-5" role="presentation">
          ...
          {navLinks.map((link) => {
            ...
            return (
              <a
                key={link.label}
                href={link.href}
                className={`relative rounded-full px-3 py-1 text-xs font-medium transition-all duration-200 whitespace-nowrap ${isActive
      ...
      {/* Mobile Menu Drawer */}
      {mobileMenuOpen && (
        <div className="pointer-events-auto md:hidden fixed inset-x-4 top-16 bg-white/95 backdrop-blur-2xl border border-[rgba(17,17,16,0.1)] rounded-2xl p-4 shadow-xl space-y-3 animate-in fade-in slide-in-from-top-2 duration-200">
